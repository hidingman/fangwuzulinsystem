	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`fangwuhuxing`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`fangwuhuxing`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`fangwuhuxing`, this.ruleForm);
						}else{
							oet = await this.$api.add(`fangwuhuxing`, this.ruleForm);
						}
						if(this.cross){
