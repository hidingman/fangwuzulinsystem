	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussfangyuanxinxi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussfangyuanxinxi`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussfangyuanxinxi`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussfangyuanxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussfangyuanxinxiCleanType',true);
						if(this.cross){
