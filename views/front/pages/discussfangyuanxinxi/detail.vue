		...
		<view class="container" :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#FFFBF3","height":"auto"}'>
			<view :style='{"width":"100%","padding":"24rpx","background":"none","height":"auto"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"10rpx 24rpx","flexWrap":"wrap","background":"linear-gradient( 113deg, #E5EFBE 0%, #EFE0BE 100%)","display":"flex","gap":"20rpx","width":"100%","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"#000","background":"none","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('discussfangyuanxinxi','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"#000","background":"none","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('discussfangyuanxinxi','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussfangyuanxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussfangyuanxinxiCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussfangyuanxinxi', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussfangyuanxinxi');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussfangyuanxinxi');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussfangyuanxinxi', this.id);
				let reg=new RegExp('http://localhost:8080/fangwuzulinsystem/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discussfangyuanxinxi',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussfangyuanxinxi', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussfangyuanxinxi',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussfangyuanxinxi/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussfangyuanxinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussfangyuanxinxi',that.detail)
							setTimeout(()=>{
