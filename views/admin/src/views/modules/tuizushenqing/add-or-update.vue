	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="退租时间" prop="tuizushijian" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.tuizushijian" 
						type="datetime"
						:readonly="ro.tuizushijian"
						placeholder="退租时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.tuizushijian" label="退租时间" prop="tuizushijian" >
					<el-input v-model="ruleForm.tuizushijian" placeholder="退租时间" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="退租原因" prop="tuizuyuanyin" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="退租原因"
					v-model="ruleForm.tuizuyuanyin" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.tuizuyuanyin" label="退租原因" prop="tuizuyuanyin" >
				<span class="text">{{ruleForm.tuizuyuanyin}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="退租详情" prop="tuizuxiangqing" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.tuizuxiangqing" 
					class="editor"
					myQuillEditor="tuizuxiangqing"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.tuizuxiangqing" label="退租详情" prop="tuizuxiangqing" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.tuizuxiangqing"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.tuizuxiangqing = this.ruleForm.tuizuxiangqing.replace(reg,'../../../fangwuzulinsystem/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `tuizushenqing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.tuizushenqingCrossAddOrUpdateFlag = false;
											this.parent.search();
