	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="评价评分" prop="pingjiapingfen" >
					<el-select :disabled="ro.pingjiapingfen" v-model="ruleForm.pingjiapingfen" placeholder="请选择评价评分" >
						<el-option
							v-for="(item,index) in pingjiapingfenOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="评价评分" prop="pingjiapingfen" >
					<el-input v-model="ruleForm.pingjiapingfen"
						placeholder="评价评分" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="公司评价" prop="gongsipingjia" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="公司评价"
					v-model="ruleForm.gongsipingjia" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.gongsipingjia" label="公司评价" prop="gongsipingjia" >
				<span class="text">{{ruleForm.gongsipingjia}}</span>
			</el-form-item>
			<el-form-item class="textarea" v-if="type!='info'" label="员工评价" prop="yuangongpingjia" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="员工评价"
					v-model="ruleForm.yuangongpingjia" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yuangongpingjia" label="员工评价" prop="yuangongpingjia" >
				<span class="text">{{ruleForm.yuangongpingjia}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="评价详情" prop="pingjiaxiangqing" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.pingjiaxiangqing" 
					class="editor"
					myQuillEditor="pingjiaxiangqing"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.pingjiaxiangqing" label="评价详情" prop="pingjiaxiangqing" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.pingjiaxiangqing"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.pingjiaxiangqing = this.ruleForm.pingjiaxiangqing.replace(reg,'../../../fangwuzulinsystem/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zulinpingjia/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zulinpingjiaCrossAddOrUpdateFlag = false;
											this.parent.search();
