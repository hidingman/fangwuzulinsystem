<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">房屋名称</label>
						<el-input v-model="searchForm.fangwumingcheng" placeholder="房屋名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">房屋地址</label>
						<el-input v-model="searchForm.fangwudizhi" placeholder="房屋地址" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">租金价格</label>
						<el-input v-model="searchForm.zujinjiagestart" placeholder="最小租金价格" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' :label="'至'">
						<el-input v-model="searchForm.zujinjiageend" placeholder="最大租金价格" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">小区</label>
						<el-input v-model="searchForm.xiaoqu" placeholder="小区" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('fangyuanxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('fangyuanxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('fangyuanxinxi','房屋户型统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"15px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="fangwutupian" width="200" label="房屋图片">
						<template slot-scope="scope">
							<div v-if="scope.row.fangwutupian">
								<img v-if="scope.row.fangwutupian.substring(0,4)=='http'&&scope.row.fangwutupian.split(',w').length>1" :src="scope.row.fangwutupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.fangwutupian)">
								<img v-else-if="scope.row.fangwutupian.substring(0,4)=='http'" :src="scope.row.fangwutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.fangwutupian.split(',')[0])">
					...
					<el-table-column :resizable='true' prop="fangwushipin" label="房屋视频">
						<template slot-scope="scope">
							<el-button v-if="scope.row.fangwushipin" type="text" size="small" @click="preClick(scope.row.fangwushipin)">预览</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="fangchanzhengbianhao"
						label="房产证编号">
						<template slot-scope="scope">
							{{scope.row.fangchanzhengbianhao}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="fangchanzhengzhaopian" width="200" label="房产证照片">
						<template slot-scope="scope">
							<div v-if="scope.row.fangchanzhengzhaopian">
								<img v-if="scope.row.fangchanzhengzhaopian.substring(0,4)=='http'&&scope.row.fangchanzhengzhaopian.split(',w').length>1" :src="scope.row.fangchanzhengzhaopian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.fangchanzhengzhaopian)">
								<img v-else-if="scope.row.fangchanzhengzhaopian.substring(0,4)=='http'" :src="scope.row.fangchanzhengzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.fangchanzhengzhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.fangchanzhengzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.fangchanzhengzhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="fangzhushenfenzheng"
						label="房主身份证">
						<template slot-scope="scope">
							{{scope.row.fangzhushenfenzheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="fangzhudianhua"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fangyuanxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('fangyuanxinxi','房屋租赁')" @click="fangwuzulinCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								房屋租赁
							</el-button>
							<el-button class="btn8" v-if="isAuth('fangyuanxinxi','预约看房')" @click="yuyuekanfangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								预约看房
							</el-button>
							<el-button class="edit" v-if=" isAuth('fangyuanxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('fangyuanxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('fangyuanxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<fangwuzulin-cross-add-or-update v-if="fangwuzulinCrossAddOrUpdateFlag" :parent="this" ref="fangwuzulinCrossaddOrUpdate"></fangwuzulin-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="fangwuhuxingChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import fangwuzulinCrossAddOrUpdate from "../fangwuzulin/add-or-update";
	...
	export default {
		data() {
			return {
				...
				fangwuzulinCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			fangwuzulinCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fangwuzulinCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fangwuzulinCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fangwuzulinCrossaddOrUpdate.init(row.id,type);
				});
			},
			yuyuekanfangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var fangwuhuxingChart1 = echarts.init(document.getElementById("fangwuhuxingChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							fangwuhuxingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								fangwuhuxingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				if(this.searchForm.fangwuhuxing!='' && this.searchForm.fangwuhuxing!=undefined){
					params['fangwuhuxing'] = this.searchForm.fangwuhuxing
				}
 				if(this.searchForm.fangwudizhi!='' && this.searchForm.fangwudizhi!=undefined){
					params['fangwudizhi'] = '%' + this.searchForm.fangwudizhi + '%'
				}
 				if(this.searchForm.zujinjiagestart!='' && this.searchForm.zujinjiagestart!=undefined ){
					params['zujinjiagestart'] = this.searchForm.zujinjiagestart
				}
				if(this.searchForm.zujinjiageend!='' && this.searchForm.zujinjiageend!=undefined){
					params['zujinjiageend'] = this.searchForm.zujinjiageend
				}
				if(this.searchForm.yajinfangshi!='' && this.searchForm.yajinfangshi!=undefined){
					params['yajinfangshi'] = this.searchForm.yajinfangshi
				}
				if(this.searchForm.fukuanfangshi!='' && this.searchForm.fukuanfangshi!=undefined){
					params['fukuanfangshi'] = this.searchForm.fukuanfangshi
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussfangyuanxinxi',query:{refid:id}});
			},
			// 预览
			preClick(file){
				if(!file){
